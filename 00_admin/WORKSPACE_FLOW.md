...
Workspace này hỗ trợ chuẩn bị, kiểm thử và quản lý dữ liệu thư viện DesignBuilder. Luồng chung áp dụng cho các workspace con như `IBST_Materials`, `IBST_Glazing` và `IBST_Construction`.
...
- `IBST_Materials`, `IBST_Glazing` và `IBST_Construction` chỉ là vùng kiểm thử.
- Không phát hành trực tiếp từ `IBST_Materials`, `IBST_Glazing` và `IBST_Construction` nếu chưa có quyết định riêng.
...
| --- | --- | --- |
| `IBST_Materials` | Kiểm thử dữ liệu vật liệu | Đang là workspace active |
| `IBST_Glazing` | Kiểm thử dữ liệu kính/glazing | Đang là workspace active |
| `IBST_Construction` | Kiểm thử dữ liệu construction | Đang là workspace active/skeleton tùy dữ liệu hiện có |
